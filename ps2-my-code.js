...
function fact(n) {
  ...
  //Lets also try DP O(n) time O(n) space
  let memo = {} 
  if (n == 0) return 1;
  memo[0] = 1;
  for (let i=1;i<=n;i++){
...
/**
  ...
  * ...
  * 
  * You can assume that the argument, n, is a positive integer. Do worry about efficiency.
  */

function fib(n) {
//DP, store vlues in an array as we move O(n) time O(n) space
  if(n==0) return 0
  if(n==1) return 1
